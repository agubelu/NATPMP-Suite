...
RESCODES_MSGS = {
    NATPMP_RESULT_OK: "The request was accepted by the router.",
    NATPMP_RESULT_OPCODE_NOT_SUPPORTED: "The router didn't recognise the requested operation",
    NATPMP_RESULT_VERSION_NOT_SUPPORTED: "The router does not support this version of the NAT-PMP protocol",
    NATPMP_RESULT_NOT_AUTHORIZED: "The client is not authorized to fulfill the request.",
    NATPMP_RESULT_INSUFFICIENT_RESOURCES: "The router is temporarily out of resources to fulfill the request.",
    NATPMP_RESULT_NETWORK_ERROR: "The router could not fulfill the request due to a network error.",
    NATPMP_RESULT_MULTIASSIGN_FAILED: "The router denied the multi-assign request, try requesting them individually.",
    NATPMP_RESULT_BAD_CERT: "The router rejected the client's certificate.",
    NATPMP_RESULT_TLS_ONLY: "The router is only accepting secure requests.",
}
...
if __name__ == "__main__":
    ...
    if "gui" in sys.argv or len(sys.argv) == 1:
        ...
        if req_norm.use_tls:
            ...
            print("Performing the initial handshake with the router...")
            ...
            try:
                ...
                try:
                    ...
                    sys.exit("Handshake was OK but the router's certificate could not be loaded.")
        ...
        if rescode != NATPMP_RESULT_OK:
            sys.exit("The router refused the operation: %s" % RESCODES_MSGS[rescode])
